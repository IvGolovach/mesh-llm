...
references and a separate static CPU candidate, without replacing Metal
outputs. Generated `SKIPPY_WORKLOAD_*` paths replace ambient configuration;
source- and executable-bound `producer.json` is verified before consumption.
Non-chat rows require both class-specific smoke and oracle evidence, not
split certification. Every selected GGUF/projector must exist in the verified
read-only lab cache; provisioning that cache is an external runner operation.

Every family row declares its workload `class` and GGUF `architecture`
separately. Cache preflight compares the target's architecture with its immutable
GGUF metadata. Only causal rows with complete split-parity policy contribute to
the architecture admission roster; a non-chat row sharing an architecture with
a causal model does not provide split-certification evidence.

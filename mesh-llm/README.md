...
- `GET`/`POST`/`DELETE /api/model-interests` for local explicit-interest submission and readback using canonical model refs
- `GET /api/model-targets` for the ranked mesh-wide advisory target view that combines explicit interest and live request demand
